...
var activeSkill = MythicActiveSkill(
    ...
    Icon()
            .setMaterial(Material.GLOW_LICHEN) // 材質
            .setModelData(0) // model
            .setDisplayName("&6幻刀劍武 &7Lv.${data.level}&8/&73") // 物品名稱
            .addLore( // 物品的 lore
                    buildList {
                        add("")
                        add("&7▪ 類型：&f主動")
                        ...
                        add("&f5秒內獲得移動速度II")
                        add("&f每秒造成五個內敵人")
                        add("&f攻擊傷害*1.5+體魄*0.5 攻擊傷害")
                        add("")
                        add("&7Lv2 ➤ &7技能傷害改為 &f攻擊傷害*1.75+體魄*1.2")
                        add("&7Lv3 ➤ &7技能傷害改為 &f攻擊傷害*2+體魄*1.5")
                        add("")
                    }
            )
}
...
HeroesSkills.getInstance().mainPathGui.registerElement(
        PathElement(
                activeSkill,
                mutableMapOf("fencing" to 33),
                {it.getSkillData("中階劍術").level >=5&&it.getSkillData("劍士步伐").level >=1}
        )
)
HeroesSkills.getInstance().skillManager.register(activeSkill);