...
var passiveSkill = PassiveMultipleStatSkill(
    ...
    5, //最大等級
    { player, data ->
        val map = mutableMapOf("MAX_MANA" to data.level*1.5,"MG_WEAPON_DAMAGE" to data.level*2.0)
        if(data.level > 4) map.put("MG_WEAPON_DAMAGE" , 15.0);
        map;
    },  //素質增加的量
    { player, data -> Icon() //底下設定物品
        .setMaterial(Material.GHAST_TEAR) // 材質
        .setModelData(0) // model
        .setDisplayName("&6圖騰信仰 &7Lv.${data.level}&8/&75") // 物品名稱
        .addLore( // 物品的 lore
            buildList {
                add("")
                add("&7▪ 類型：&f被動")
                ...
                add("&f守護神帶來的力量就越多")
                add("")
                add("&7Lv1-5 ➤ 增加 &f1.5 &8(${data.level*1.5}) &7最大魔力")
                add("&7Lv1-5 ➤ 增加 &f2 &8(${data.level*2}) &7攻擊魔傷")
                add("&7Lv5 ➤ 額外增加 &f15 &7攻擊魔傷")
                add("")
            }
        )
    }
...
HeroesSkills.getInstance().mainPathGui.registerElement(
    PathElement(
        passiveSkill,
        mutableMapOf("totem" to 22),
        {it.getSkillData("圖騰之力").level >=1}
    )
)
